...
- When the owner leaves, the code expires and remaining players return to the hub after `ownerLeaveGracePeriod` (10 minutes by default). If the owner comes back within it, the countdown is called off and everyone else is told, with `personalServers.ownerReturned`.
